...
\documentclass[xcolor=dvipsnames, mathserif, 10pt, aspectratio=169, handout]{beamer}
...
\author[\href{https://gdgarcia.ca}{Garcia (Université Laval)}]{Guilherme D.\ Garcia\vspace{-1ex}}
...
\begin{itemize}
	\item Votre nom, programme
	\item Un intérêt non académique?
\end{itemize}
...
\begin{frame}[t]{LNG-1100}{Familiarisez-vous avec la page du cours}
...
\begin{column}{0.2\textwidth}
	...
	\item[]
	\item[] 
	\only<2->{\item[\winner] Matériels?}
	\only<2->{\item[\winner] Évaluation?}
...
\begin{center}
	\fbox{\includegraphics[width = 0.8\textwidth]{monPortail.png}}
\end{center}
\end{column}
...
\begin{important}
	\begin{itemize}
		...
		\pause
		\item[\winner] Si un petit nombre de personnes très riches vivent dans la ville, la moyenne peut être fortement biaisée. La médiane ou le mode pourraient fournir une image plus précise du revenu typique
	\end{itemize}
\end{important}
...
\begin{frame}{Exemple 6}{Quel est le problème ici?}

\begin{itemize}
	\item Observez les moyennes des deux groupes ci-dessous. Sont-ils différents?
	\item[]
\end{itemize}
...
\begin{important}
	\begin{itemize}
		\item[] \textbf{Un test médical avec une précision de 95\% a donné un résultat positif. Donc, le patient a 95\% de chances d'avoir la maladie}
		\pause
		\item[\winner] Sans tenir compte de la prévalence de la maladie dans la population, cette affirmation peut être trompeuse. Si la maladie est rare (p.\ ex., 1/1000), un résultat positif peut toujours être un faux positif
	\end{itemize}
\end{important}
...
\begin{itemize}
	\item On commence à travailler avec RStudio à partir de la semaine prochaine
	\item[\winner] Consultez la page du cours et les matériels \lav{avant} notre séance
	\item[]
\end{itemize}
...
\begin{center}
	\begin{tikzpicture}[->]
		\Tree[.{\lav{R} : langage + EDI\footnote{Environnement de développement intégré.} (+ ses \lav{extensions})}
			[.{\fbox{RStudio : l'EDI}}
				[.{Analyse de données} ]
...
\pause
	\item Poser une question (et une hypothèse?)
\pause
...
\begin{itemize}
	\item[1.] \textbf{Sujet} : la production et la perception des voyelles antérieures arrondies en français \ur{y {\o} \oe}\footnote{\mylink{https://fr.wikipedia.org/wiki/Alphabet_phon\%C3\%A9tique_international}{Alphabet phonétique international}}

\pause

	\begin{itemize}
		\item l\lav{u}nette - n\lav{eu}tre - j\lav{eu}ne $\leftarrow$ très difficile (!)
	\end{itemize}
...
\pause
	...
	\begin{itemize}
		...
		\item Parfois, le projet sera exempté (bonne chance!)
		\item Alternative : utiliser des données déjà disponibles (p.\ ex., en ligne)
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Sondage (MS Forms et Google Forms : gratuits et faciles) $\leftarrow$ \red{Notre point de départ ce semestre}
	\pause
...
\begin{frame}{Exemples additionnels}{\fbox{choix forcé}}
	...
	\end{itemize}
	\begin{center}
{\faVolumeUp} [\ipaa{"kI.mE.s@r}] \hspace{3em} {\faVolumeUp} [\ipaa{kI."mE.s@r}]
	\end{center}
...
\begin{itemize}
	\item[] Consultez les \fr{premiers pas} (Nouvelles) sur monPortail :
	\item Lisez \citet[ch.\ 1--7]{barnier_R}
 	\item Installez R + RStudio + \var{tidyverse}
 	\item[]
 	\item Si vous avez de problèmes :
 	\begin{enumerate}
 		\item Forum sur monPortail
 		\item ChatGPT (mais consultez le module \fr{Conseils sur ChatGPT} sur monPortail)
 		\item Alternative : \mylink{posit.cloud}{http://posit.cloud} (version en ligne)
 	\end{enumerate} 
\end{itemize}
