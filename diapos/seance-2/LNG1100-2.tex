...
\begin{itemize}
	\item Base de données générale (plusieurs domaines/sujets) : \mylink{https://www.iris-database.org}{iris-database.org}
\end{itemize}
...
\begin{center}
	\fbox{\includegraphics[width = 0.6\textwidth]{atlas}}
\end{center}
...
\begin{itemize}
	\item On concentre tous les fichiers du cours dans un seul dossier
	\item RStudio connaîtra déjà la localisation des fichiers
	\item[]
	\item[] Maintenant, on continue sur RStudio (consultez les fichiers plus tard) 
	\item[\winner] Voici une suggestion d'organisation :
\end{itemize}
...
-- +(0,-8pt) -| (\tikzchildnode)}}}
		\Tree[.{\faFolderOpen { LNG-1100}}
			[.LNG-1100.RProject ]
			[.{{\faFolderOpen} Données} ]
			[.{{\faFolderOpen} Séances} 
%				[.{{\faFolderOpen} LNG-1100-1} ]
...
\begin{frame}{Quelques ajustements dans RStudio}{Tools $>$ Global Options...}

\begin{center}
	\fbox{\includegraphics[width = 0.4\textwidth]{rstudio1.png}}\hspace{2em}
	\fbox{\includegraphics[width = 0.4\textwidth]{rstudio2.png}}
\end{center}
...
# Le « pipe » (|>) combine les fonctions d'une façon intuitive (raccourci Cmd/Ctrl + Shift + M) :
donnees |>
...
\begin{itemize}
	\item[\winner] Consulter les scripts et les solutions \mylink{https://fr.gdgarcia.ca/lng-1100/1-intro_a_r}{ici} 
\end{itemize}
