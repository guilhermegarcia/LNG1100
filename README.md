...
`diapos`, `donnees`, `quarto` et `scripts`. Les dossiers `diapos` et `scripts`
se remplissent au fil des séances : au début du trimestre, ils ne contiennent
qu'un fichier `README.md`.
...
```r
read_csv("donnees/base/villes.csv") # cette commande exige l'installation de Tidyverse
```
...
3. que `file.exists("donnees/base/villes.csv")` retourne `TRUE`;
